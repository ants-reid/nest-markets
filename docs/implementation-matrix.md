...
| API-RX05B | `app/api/routes/cockpit_eod_report.py` | cockpit EOD report route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-05 as a read-only `GET /cockpit/eod-report` surface that summarizes paper orders, positions, incidents, and signal-outcome lessons without calling any broker or live execution path. |
...
| API-RX05D | `app/api/routes/cockpit_trade_close_explanations.py` | cockpit trade-close explanations route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-08 as a read-only `GET /cockpit/trade-close-explanations` surface for recent closed paper trades with deterministic close labels, setup-match context, evidence, and explicit `is_actionable=false`. |
| API-RX05E | `app/api/routes/cockpit_daily_scoreboard.py` | cockpit daily scoreboard route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-09 as a read-only `GET /cockpit/daily-scoreboard` surface for daily paper performance, activity, contributors, and review priorities with explicit safe unknown/limitation handling. |
| API-RX05F | `app/api/routes/cockpit_alerts_needing_attention.py` | cockpit alerts-needing-attention route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-10 as a read-only `GET /cockpit/alerts-needing-attention` surface for active alerts, unresolved incidents, monitor degradation, stale-data warnings, and risk/halt context with explicit `is_actionable=false`. |
...
| WEB-PX09A | `app/cockpit/eod-report/page.tsx` | `/cockpit/eod-report` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-05 as a paper-only, read-focused EOD report surface with summary cards, P&L/readiness sections, incident and monitor notes, lessons, and mobile-safe mocked Playwright coverage. |
| WEB-PX09B | `app/cockpit/in-flight-adjustments/page.tsx` | `/cockpit/in-flight-adjustments` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-07 as a paper-only, read-focused in-flight adjustments surface with summary cards, attention table, detailed reason/evidence sections, monitor/risk notes, and responsive no-overflow browser coverage. |
