...
| Mode | Primary route/service seam | execution_source | balance_source | fees_source | fills_source | positions_source | serious_paper_source | is_canonical_paper | Submission capability | Current lock state |
|---|---|---|---|---|---|---|---|---|---|---|
| internal_mock_simulator | API-R06 `/execution/paper` + API-S05 `paper_execution_service.py` + API-P05 persistence layer | internal_mock_simulator | app_simulated | estimated | simulated | app_db_simulated | ibkr_paper | false | Simulated lifecycle only (`submit/fill/close` in app) | Not live-capable |
...
- This contract is clarity/routing metadata only and does not unlock live submission.
...
| API-RX05B | `app/api/routes/cockpit_eod_report.py` | cockpit EOD report route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-05 as a read-only `GET /cockpit/eod-report` surface that summarizes paper orders, positions, incidents, and signal-outcome lessons without calling any broker or live execution path. Updated 2026-05-22 during MH-COCKPIT-11 to include optional asset-context deep-link metadata (`asset_id`, `asset_name`, `asset_detail_path`, `has_asset_context`) for navigation-only flows. |
| API-RX05C | `app/api/routes/cockpit_in_flight_adjustments.py` | cockpit in-flight adjustments route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-07 as a read-only `GET /cockpit/in-flight-adjustments` surface for paper in-flight positions, orders, recommendations, risk notes, and monitor notes with `is_actionable=false` for every item. Updated 2026-05-22 during MH-COCKPIT-11 to return optional asset-context deep-link metadata for navigation-only visibility. |
| API-RX05D | `app/api/routes/cockpit_trade_close_explanations.py` | cockpit trade-close explanations route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-08 as a read-only `GET /cockpit/trade-close-explanations` surface for recent closed paper trades with deterministic close labels, setup-match context, evidence, and explicit `is_actionable=false`. Updated 2026-05-22 during MH-COCKPIT-11 to include optional asset-context deep-link metadata for review navigation. |
| API-RX05E | `app/api/routes/cockpit_daily_scoreboard.py` | cockpit daily scoreboard route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-09 as a read-only `GET /cockpit/daily-scoreboard` surface for daily paper performance, activity, contributors, and review priorities with explicit safe unknown/limitation handling. Updated 2026-05-22 during MH-COCKPIT-11 so top contributor rows can include optional asset-context deep-link metadata. |
| API-RX05F | `app/api/routes/cockpit_alerts_needing_attention.py` | cockpit alerts-needing-attention route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-10 as a read-only `GET /cockpit/alerts-needing-attention` surface for active alerts, unresolved incidents, monitor degradation, stale-data warnings, and risk/halt context with explicit `is_actionable=false`. Updated 2026-05-22 during MH-COCKPIT-11 with optional asset-context metadata on items where asset identity can be resolved safely. |
...
| API-RX17 | `app/api/routes/paper_recommendations.py` | paper recommendations route surface | implemented | tested | documented | WS-01 | Inventoried 2026-05-19 during MH-RESTART-004. Updated 2026-05-22 during MH-BROKER-PAPER-CANONICAL-04 with read-only `GET /paper/recommendations/{recommendation_id}/serious-paper-route-check` operator eligibility checks for the canonical manual IBKR paper path. Updated 2026-05-23 with guarded `POST /paper/recommendations/{recommendation_id}/broker-dry-run-preview`, which reuses broker dry-run semantics only after route-check eligibility, persists dry-run decisions without submit behavior, and stays fail-closed in live or missing-context states. |
...
| API-RX28 | `app/api/routes/monitor_test.py` | monitor operator dry-probe route surface | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-MON-10 and hardened 2026-05-22 during MH-MON-09 as auth-gated `POST /monitor/test/{service_id}` with fail-closed unknown/unsupported handling plus cooldown-aware abuse resistance. Route remains dry-probe-only and never mutates trading state. |
...
| API-SX02 | `app/services/asset_card_service.py` | asset card service | implemented | tested | partial | WS-01 | Inventoried 2026-05-19 during MH-RESTART-004. Validation baseline was re-confirmed on 2026-05-23 after narrowing `tests/test_asset_card_service.py::test_inactive_excluded_by_default` to a deterministic asset-class slice; production service logic did not change. |
...
| API-SX07 | `app/services/broker_service.py` | broker service | implemented | tested | documented | WS-01 | Updated 2026-05-22 during MH-161 as a stable facade over split internal helpers (`broker_preflight_decision_service.py`, `broker_preflight_advisory_service.py`) with no route/API behavior change. MH-162 added post-lock regression coverage (`apps/api/tests/test_post_lock_simulation_regression.py`) for source-model separation, fail-closed preflight semantics, live-lock submit auditing, and worker async/auto-submit seam invariants. |
...
| API-SX09B | `app/services/cockpit_eod_report_service.py` | cockpit EOD report service | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-05 as a deterministic read-only aggregator over paper orders, paper positions, incident logs, and signal outcomes. Unsupported metrics remain `null`/limited instead of being invented. Updated 2026-05-22 during MH-COCKPIT-11 to emit canonical asset-detail path metadata for open/closed position review navigation. |
| API-SX09C | `app/services/cockpit_in_flight_adjustments_service.py` | cockpit in-flight adjustments service | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-07 as a deterministic read-only aggregator over open paper positions, active paper orders, active paper recommendations, risk decisions, and monitor incidents, with safe unknown/limitation handling and no execution-side calls. Updated 2026-05-22 during MH-COCKPIT-11 to emit optional asset-context deep-link metadata per item where identity is resolvable. The 2026-05-23 guarded broker dry-run review work reused this read-only surface for recommendation-owned route-check and preview review without adding any submit or execution authority. |
| API-SX09D | `app/services/cockpit_trade_close_explanations_service.py` | cockpit trade-close explanations service | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-08 as a deterministic read-only aggregator over closed paper positions plus linked paper orders, signal outcomes, and risk decisions to infer safe close labels and audit context without any execution-side calls. Updated 2026-05-22 during MH-COCKPIT-11 with asset-context deep-link metadata for navigation-only review. |
| API-SX09E | `app/services/cockpit_daily_scoreboard_service.py` | cockpit daily scoreboard service | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-09 as a deterministic read-only aggregator over paper orders, paper positions, risk decisions, and monitor incidents to produce daily scoreboard metrics without broker/live/mutation calls. Updated 2026-05-22 during MH-COCKPIT-11 to include optional contributor asset-context path metadata. |
| API-SX09F | `app/services/cockpit_alerts_attention_service.py` | cockpit alerts-needing-attention service | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-10 as a deterministic read-only aggregator over persisted alert projections, incident logs, monitor probes, stale paper-order scans, risk-limit status, and trading-halt status with safe unknown/limitation handling and no mutation calls. Updated 2026-05-22 during MH-COCKPIT-11 to resolve and return optional asset-context metadata for alert/notification/paper-order items when evidence exists. |
...
| API-SX37A | `app/services/paper_recommendation_broker_dry_run_preview_service.py` | recommendation-owned guarded broker dry-run preview service | implemented | tested | documented | WS-01 | Added 2026-05-23 during the Manual Guarded Broker Dry-Run Review Surface phase to require recommendation route-check eligibility before building a broker dry-run request, reuse persisted dry-run decision rows when available, emit additive source/preflight metadata, and keep submit/live/worker behavior unchanged. |
...
| API-SX79 | `app/services/monitor_test_service.py` | monitor dry-probe execution service | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-MON-10 and hardened in MH-MON-09 with per-service cooldown, timeout boundary, recursive secret scrubbing, safe non-leaky error messaging, and append-only incident logging for operator dry-probe attempts. |
...
| WEB-PX09A | `app/cockpit/eod-report/page.tsx` | `/cockpit/eod-report` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-05 as a paper-only, read-focused EOD report surface with summary cards, P&L/readiness sections, incident and monitor notes, lessons, and mobile-safe mocked Playwright coverage. Updated 2026-05-22 during MH-COCKPIT-11 with navigation-only asset-context links to the canonical asset-detail route. |
| WEB-PX09B | `app/cockpit/in-flight-adjustments/page.tsx` | `/cockpit/in-flight-adjustments` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-07 as a paper-only, read-focused in-flight adjustments surface with summary cards, attention table, detailed reason/evidence sections, monitor/risk notes, and responsive no-overflow browser coverage. Updated 2026-05-22 during MH-COCKPIT-11 with navigation-only asset-context links and safe unavailable fallback text. Updated 2026-05-23 with an operator-facing recommendation route-check review panel that reads the existing backend contract, stays non-submitting, and links only to the guarded `/broker` dry-run/manual paper flow. Updated again on 2026-05-23 with a recommendation-owned guarded broker dry-run preview panel that appears only for eligible route-check results, never exposes submit controls, and remains validation-backed after clean stale-chunk runtime recovery on `127.0.0.1:3100`. Updated again on 2026-05-23 with a frontend-only manual IBKR paper submit readiness review section derived from existing route-check and guarded dry-run evidence, explicitly non-submitting, with live still locked and workers still unable to submit. Updated again on 2026-05-23 with a frontend-only manual paper submit handoff review section that exposes future guarded `/broker/orders` handoff requirements, required future payload fields, submit-time safety gates, current blockers, and explicit no-submit/no-order-submitted copy without adding any submit call or control. Updated again on 2026-05-23 with a frontend-only manual paper submit audit package section that consolidates recommendation identity, route-check, dry-run, readiness, handoff, source labels, decision references, safety state, payload preview fields, and next-action review evidence without adding any submit button, submit call, or `/broker/orders` request from the cockpit surface. Updated again on 2026-05-23 with a frontend-only manual paper submit approval package section that derives approval-only evidence, future manual approval requirements, audit references, submit-time safety reruns, and explicit no-submit/no-order-submitted/live-locked/worker-non-submitting posture from the existing review chain without adding any submit button, submit call, or `/broker/orders` request from the cockpit surface. Updated again on 2026-05-23 with a frontend-only guarded manual paper submit preflight contract section that derives the final pre-submit checklist, submit-time rerun requirements, decision-logging requirements, source-label rechecks, stale-data checks, final payload review fields, and operator confirmations from the existing review chain without adding any submit button, submit call, or `/broker/orders` request from the cockpit surface. Updated again on 2026-05-23 with a frontend-only future manual submit design review section that documents the future `/broker/orders` seam, existing backend owner, future cockpit host, submit-time rerun checks, decision logging, worker restrictions, and intentionally not implemented items without adding any submit button, `/broker/orders` request, or new `BrokerService.submit_order` path from the cockpit surface. Updated again on 2026-05-23 with a frontend-only guarded operator submit-decision review section that maps existing decision writers and readers, future manual_paper_submit decision records, persisted field requirements, fail-closed rules, and linked review evidence without adding any decision write, submit call, `/broker/orders` UI call, or new `BrokerService.submit_order` path from the cockpit surface. Updated again on 2026-05-23 with a frontend-only guarded operator action review section that maps the future `manual_ibkr_paper_submit` action, later guarded `/broker/orders` route use, final operator confirmations, final payload preview fields, submit-time reruns, future decision records, and unavailable-state rules while keeping `action_available_now=false`, adding no submit button, adding no `/broker/orders` UI call, and adding no new `BrokerService.submit_order` path from the cockpit surface. Updated again on 2026-05-23 with a frontend-only final guarded operator submit interaction spec section that explains how a later approved `manual_ibkr_paper_submit` interaction would flow through guarded `/broker/orders`, keeps `action_available_now=false`, writes no decision now, submits no order, reruns submit-time checks later, and adds no submit button, `/broker/orders` UI call, or new `BrokerService.submit_order` path from the cockpit surface. Updated again on 2026-05-23 with a submit-readiness architecture checkpoint that records `/broker/orders` in `broker.py` and `BrokerService.submit_order` as the only serious-paper submit seam, keeps the cockpit panel on recommendation-owned read-only helpers only, recommends a future dedicated guarded confirmation surface instead of direct inline execution, preserves the existing `OrderRequestSchema` payload contract, requires submit-time reruns and append-only decision persistence, and adds a static drift test proving `RecommendationRouteCheckPanel.tsx` does not import `submitBrokerOrder`. Updated again on 2026-05-23 with a navigation-only link into the dedicated confirmation route so operators can inspect the future guarded paper submit surface without adding any execution control or `/broker/orders` UI call. Updated again on 2026-05-24 to centralize the cockpit panel onto the shared frontend-only `manualPaperSubmitReview` derivation helper already used by the dedicated confirmation surface, removing the active review-chain drift without adding any submit path. Updated again on 2026-05-24 with a compact in-flight payload freshness summary that reuses the shared freshness helper, reads recommendation timestamps plus route-check/dry-run observation time, surfaces rerun requirements and missing/stale evidence compactly, and keeps submit disabled with no order submitted, live still locked, and workers still non-submitting. Updated again on 2026-05-24 with a compact missing-context triage summary that reuses the shared triage helper, groups payload, route-check, dry-run, approval/preflight, freshness, source-label, broker-mode, and blocking review gaps, and keeps the panel read-only with submit disabled, live locked, and workers unable to submit. The 2026-05-24 go/no-go checkpoint kept this surface review-only and explicitly recorded that any first executable manual-paper submit phase remains blocked on missing confirmation-control safety tests, not on a missing backend seam. Updated again on 2026-05-24 in the safety-tests-only follow-up to keep the in-flight Playwright coverage aligned with the current blocked-by-review triage precedence and locked/live semantics while preserving the panel’s read-only, non-submitting posture. |
| WEB-PX09F | `app/cockpit/manual-paper-submit-confirmation/page.tsx` | `/cockpit/manual-paper-submit-confirmation` | implemented | tested | documented | WS-01 | Added 2026-05-23 as a dedicated design-only confirmation surface for a future guarded manual IBKR paper submit. The page reads existing recommendation route-check and guarded broker dry-run evidence only, renders future route/payload/submit-time rerun/decision-persistence/blocking-state/confirmation-wording visibility, keeps `submit_enabled_now=false`, `order_submitted=false`, live locked, and workers non-submitting, and exposes only a disabled placeholder button labelled `Submit not enabled in this phase`. Updated again on 2026-05-23 to derive and display the same read-only readiness, handoff, audit, approval, preflight, design-review, submit-decision-review, operator-action-review, and final-interaction-spec statuses already used by the in-flight review chain, while preserving safe no-context rendering and adding a shared frontend-only derivation helper that remains submit-free. Updated again on 2026-05-24 with a shared read-only payload freshness review that compares the reviewed recommendation payload against the current route-check and guarded dry-run evidence, fails closed on missing timestamps, surfaces rerun requirements for route-check/dry-run/approval/preflight evidence, and keeps submit disabled with live locked and workers non-submitting. Updated again on 2026-05-24 with a full shared missing-context triage section that groups payload, route-check, dry-run, approval/preflight, freshness, source-label, broker-mode, and blocking review gaps while keeping submit disabled, order submission absent, live locked, and workers non-submitting. The 2026-05-24 go/no-go checkpoint confirmed this route is still the correct future executable host, but recorded a `NOT_READY_MISSING_SAFETY_TESTS` verdict because the future confirmation-control enablement cases are not yet covered. Validation includes the dedicated Playwright spec, targeted `routes.spec.ts` and `responsive.spec.ts` coverage, a static drift test proving the page and shared helper do not import `submitBrokerOrder`, and a clean Next production build after wrapping the `useSearchParams()` consumer in `Suspense`. Updated again on 2026-05-24 in the safety-tests-only follow-up with additional confirmation-surface coverage for stale reviewed recommendation evidence, explicit no-final-confirmation-control/no-submit behavior, and backend route-level append-only decision persistence on the guarded `/broker/orders` seam, still with no submit implementation, no `/broker/orders` UI call, and no `submitBrokerOrder` import. |
| WEB-PX09C | `app/cockpit/trade-close-explanations/page.tsx` | `/cockpit/trade-close-explanations` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-08 as a paper-only, read-focused trade-close explanations surface with summary cards, close-label table, detailed evidence/missing-data cards, and responsive no-overflow mocked Playwright coverage. Updated 2026-05-22 during MH-COCKPIT-11 with asset-context deep links to asset detail where identity is known. |
| WEB-PX09D | `app/cockpit/daily-scoreboard/page.tsx` | `/cockpit/daily-scoreboard` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-09 as a paper-only, read-focused daily scoreboard surface with day-status badge, activity/performance panels, contributors, notes, priorities, and responsive no-overflow mocked Playwright coverage. Updated 2026-05-22 during MH-COCKPIT-11 with contributor-level asset-context links and safe no-context fallback. |
| WEB-PX09E | `app/cockpit/alerts-needing-attention/page.tsx` | `/cockpit/alerts-needing-attention` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-10 as a paper-only, read-focused attention surface with priority/source grouping, item evidence/missing-data cards, monitor/risk notes, limitations, and responsive no-overflow mocked Playwright coverage. Updated 2026-05-22 during MH-COCKPIT-11 with resolved asset-context links for alert/notification/order items and safe unavailable fallback for non-asset rows. |
...
| QA-T15 | `apps/api/tests/routes` | backend route and safety route coverage | implemented | tested | documented | WS-05 | Route suites cover broker, broker health, risk limits, trading halt, paper recommendations, and broker audit paths; refreshed full backend pytest rerun green on 2026-05-23 after adding recommendation-owned guarded broker dry-run preview coverage and route-registry drift updates. |
