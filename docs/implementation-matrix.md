...
| QA-T20 | `apps/api/tests/test_response_model_catalog_drift_lock.py` | response-model catalog drift-lock coverage | implemented | tested | documented | WS-05 | Added 2026-05-31. Pins safety-route `response_model` bindings on `execution.py`, `workflow.py`, and (extended 2026-05-31) `broker_submit_decisions.py`. Catches cross-file response-model swaps for `/paper`, `/live`, `/run`, and `/submit-decisions/recent`. 3/3 tests green. |
