...
| API-RX03 | `app/api/routes/broker.py` | broker route surface | implemented | tested | documented | WS-01 | Inventoried 2026-05-19 during MH-RESTART-004. Updated 2026-05-22 during MH-BROKER-PAPER-CANONICAL-03 with read-only `GET /broker/paper/canonical-route` routing resolution for operational serious-paper workflows. Updated again on 2026-05-24 after the guarded manual paper submit rollout and safety audit: `POST /broker/orders` remains the only serious-paper submit seam, returns structured `paper_preflight_blocked` detail for blocked paper submits, and continues to publish canonical paper-route metadata with live still locked. |
| API-RX04 | `app/api/routes/broker_submit_decisions.py` | broker submit decisions route surface | implemented | tested | documented | WS-01 | Updated 2026-05-22 during MH-148-C + MH-147 with active writer-backed audit feed and route/service test coverage. Updated again on 2026-05-24 during Broker Submit Decision Timeline / Paper Submit Result History to return a typed safe timeline response, expose extracted top-level decision fields plus request summary, and support read-only filters for source, decision status, correlation id, and recommendation id. |
...
| API-SX07 | `app/services/broker_service.py` | broker service | implemented | tested | documented | WS-01 | Updated 2026-05-22 during MH-161 as a stable facade over split internal helpers (`broker_preflight_decision_service.py`, `broker_preflight_advisory_service.py`) with no route/API behavior change. MH-162 added post-lock regression coverage (`apps/api/tests/test_post_lock_simulation_regression.py`) for source-model separation, fail-closed preflight semantics, live-lock submit auditing, and worker async/auto-submit seam invariants. Updated again on 2026-05-24 after guarded manual paper submit enablement to confirm the paper submit path reruns trading-control and dry-run preflight checks at submit time, blocks on `would_block` and `blocked` decisions, and persists append-only `submit_preflight` plus `submit_attempt` records for both blocked and allowed attempts. |
...
| WEB-PX09B | `app/cockpit/in-flight-adjustments/page.tsx` | `/cockpit/in-flight-adjustments` | implemented | tested | documented | WS-01 | Added 2026-05-22 during MH-COCKPIT-07 as a paper-only, read-focused in-flight adjustments surface with summary cards, attention table, detailed reason/evidence sections, monitor/risk notes, and responsive no-overflow browser coverage. Updated 2026-05-22 during MH-COCKPIT-11 with navigation-only asset-context links and safe unavailable fallback text. Updated 2026-05-23 with an operator-facing recommendation route-check review panel that reads the existing backend contract, stays non-submitting, and links only to the guarded `/broker` dry-run/manual paper flow. Updated again on 2026-05-23 with a recommendation-owned guarded broker dry-run preview panel that appears only for eligible route-check results, never exposes submit controls, and remains validation-backed after clean stale-chunk runtime recovery on `127.0.0.1:3100`. Updated again on 2026-05-23 with the shared readiness, handoff, audit, approval, preflight, design-review, submit-decision-review, operator-action-review, and final-interaction review chain, all still read-only. Updated again on 2026-05-24 to centralize the cockpit panel onto the shared frontend-only `manualPaperSubmitReview` derivation helper already used by the dedicated confirmation surface, add compact freshness and missing-context triage summaries, and preserve the non-submitting boundary. Post-implementation safety audit on 2026-05-24 reconfirmed that this surface remains review-only, does not import `submitBrokerOrder`, does not call `/broker/orders`, keeps live locked and workers non-submitting, and routes executable submit only to the dedicated confirmation host. |
| WEB-PX09F | `app/cockpit/manual-paper-submit-confirmation/page.tsx` | `/cockpit/manual-paper-submit-confirmation` | implemented | tested | documented | WS-01 | Added 2026-05-23 as the dedicated confirmation host for guarded manual IBKR paper submit. The page derives route-check, guarded broker dry-run, freshness, and missing-context review evidence from the shared helper, requires explicit final confirmation, fails closed when any review or routing gate is not clear, and is the only cockpit surface allowed to import `submitBrokerOrder`. Updated on 2026-05-24 to activate the guarded paper-only submit control on this route only: the control submits exclusively to `/broker/orders`, sends `account_mode=paper` and `execution_source=ibkr_paper`, renders structured blocked-submit detail from backend `paper_preflight_blocked` responses, reports success state after broker acceptance, and preserves live-locked plus worker-non-submitting posture. Responsive overflow fixes, dedicated Playwright coverage, boundary drift tests, backend broker-route/service tests, and the post-implementation safety audit all confirmed that this route is now the sole executable cockpit host while `/execution/paper` remains outside the serious-paper submit seam. Extended on 2026-06-01 with an `OperatorOutcomeView` (`data-testid="manual-paper-submit-outcome-view"`) that renders post-attempt: outcome status (`allowed`/`blocked`/`failed`), persistent safety badges (Paper only, Live remains locked, Workers cannot submit, No live order was placed), attempt details (symbol, side, qty, order type, TIF, est. notional, recommendation id, correlation id, attempted_at), guard result fields (broker mode, broker account mode, execution source, preflight decision status, dry-run allowed_to_submit / would_block, response status, broker order id, filled qty/price, live_execution_enabled, paper_trading_enabled, and on failure `submit_gate`/`response_decision_status`), blocked reasons list, a safe warning paragraph, a read-only link to `/cockpit/audit/broker-submit-decisions?correlation_id=…&recommendation_id=…`, and outcome-specific next-step guidance with explicit "No automatic resubmission" copy. No new submit route or behaviour was added; `/broker/orders` remains the only serious-paper submit seam, live remains locked, workers remain non-submitting. |
| WEB-PX09G | `app/cockpit/audit/broker-submit-decisions/page.tsx` | `/cockpit/audit/broker-submit-decisions` | implemented | tested | documented | WS-01 | Updated 2026-05-24 during Broker Submit Decision Timeline / Paper Submit Result History to turn the existing audit reader into a read-only decision timeline/history surface with summary cards, source/status filters, safe request/routing metadata, blocked-reason and warning lists, and direct cockpit hub navigation. Validation includes a dedicated mocked Playwright spec plus targeted route-render and responsive coverage. |
...
| QA-T19 | `apps/api/tests/{test_broker_submit_decision_timeline_route_surface_drift_lock.py,test_broker_submit_decision_timeline_frontend_drift_lock.py}` | broker submit decision timeline drift-lock coverage | implemented | tested | documented | WS-05 | Added 2026-05-31. Pins GET-only `/broker/submit-decisions/recent` route surface, response_model binding, filter signature, 5 Pydantic timeline schema field catalogs, no-secret-fields scan, no-submit-seam import scan; plus frontend static pins that the cockpit timeline page and client helper remain read-only and never reference `/broker/orders` or `/execution/paper`. Extended 2026-05-31 to add SHA-256 body pins on `list_recent_broker_submit_decisions` and `getRecentBrokerSubmitDecisions`, and to pin the cockpit-audit landing page (`apps/web/app/cockpit/audit/page.tsx`) link to `/cockpit/audit/broker-submit-decisions`. Extended again 2026-05-31 to SHA-256-pin the timeline page body (`apps/web/app/cockpit/audit/broker-submit-decisions/page.tsx`) and pin the cockpit audit-hub broker-submit tile's count contract to the audit envelope's `count` field. 30/30 tests green; full backend pytest 2437/2437 green. |
| QA-T20 | `apps/api/tests/test_response_model_catalog_drift_lock.py` | response-model catalog drift-lock coverage | implemented | tested | documented | WS-05 | Added 2026-05-31. Pins safety-route `response_model` bindings on `execution.py`, `workflow.py`, and (extended 2026-05-31) `broker_submit_decisions.py`. Extended 2026-06-01 to also scan `risk_decisions.py`, `news_in_decision_log.py`, `llm_logs.py`, and `monitor_worker_run_log.py`; expected entries `RiskDecisionAuditResponseSchema`, `NewsInDecisionLogAuditResponseSchema`, `LlmLogAuditResponseSchema`, `WorkerRunLogOverviewResponseSchema`. Catches cross-file response-model swaps for `/paper`, `/live`, `/run`, `/submit-decisions/recent`, `/risk-decisions/recent`, `/news-in-decision-log/recent`, `/llm-logs/recent`, and `/monitor/worker-run-log/overview`. 3/3 tests green. |
| QA-T21 | `apps/api/tests/test_audit_feed_response_model_no_secret_drift_lock.py` | cockpit audit feed no-secret + envelope coverage | implemented | tested | documented | WS-05 | Added 2026-06-01. Forbids credential-shaped field names (`password`, `secret`, `api_key`, `access_token`, `refresh_token`, `bearer_token`, `authorization`, `private_key`, `cookie`, …) on every schema in `apps/api/app/schemas/audit_feeds.py`; pins the top-level envelope key set for the four audit-feed response schemas. 2/2 tests green; full backend pytest 2439/2439 green. |
