...
| QA-016 | Cockpit and monitor build | cockpit, audit, and monitor pages compile in the production Next build | automated | passing | WEB-PX07, WEB-PX08, WEB-PX09, WEB-PX10, WEB-PX11, WEB-PX12, WEB-PX13, WEB-PX14, WEB-PX15, WEB-PX16, WEB-PX17, WEB-PX25, WEB-PX26, WEB-PX31 | `cd apps/web && npm run lint && npm run build` passed on 2026-05-20, including the MH-COCKPIT-03 `/cockpit` mode-selector refresh and the existing `/monitor/feeds` route |
...
| QA-125 | Monitor and audit APIs | cockpit, monitor, llm-log, broker-submit-decision, news-in-decision-log, risk-decision, feed-monitor, cockpit-mode, and monitor-test routes remain safely pinned and non-mutating | automated | passing | API-RX04, API-RX05, API-RX05A, API-RX06, API-RX08, API-RX11, API-RX12, API-RX13, API-RX15, API-RX22, API-RX27, API-RX28 | Covered by route tests, audit response-shape drift locks, targeted `/monitor/feeds` pytest coverage, MH-MON-10 route/service tests, and focused backend reruns including router drift locks |
