...
| QA-149 | Manual paper submit confirmation surface design | `/cockpit/manual-paper-submit-confirmation` renders a dedicated future guarded manual IBKR paper submit confirmation surface that remains design-only, derives from existing recommendation route-check and guarded broker dry-run evidence only, shows the future `/broker/orders` seam plus payload preview, submit-time rerun checklist, blocking states, decision persistence requirements, and confirmation wording preview, reuses the shared frontend-derived readiness, handoff, audit, approval, preflight, design-review, submit-decision-review, operator-action-review, and final-interaction-spec statuses used by the in-flight cockpit panel where evidence is available, keeps `submit_enabled_now=false`, keeps `order_submitted=false`, keeps live locked, keeps workers non-submitting, renders only a disabled placeholder button, and never calls `/broker/orders` or imports `submitBrokerOrder` | automated | passing | WEB-PX09F, WEB-PX09B, API-RX03, API-RX17, API-SX37A | Covered by `apps/web/tests/manual-paper-submit-confirmation.spec.ts`, targeted `apps/web/tests/routes.spec.ts` and `apps/web/tests/responsive.spec.ts` reruns on `127.0.0.1:3100`, `apps/api/tests/test_recommendation_route_check_panel_submit_boundary.py`, and `cd apps/web && npm run lint && npm run build` on 2026-05-23. |
