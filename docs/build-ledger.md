...
## MH-FEED-MONITOR-002-R1 — Reboot Recovery And Current Validation

**Date**: 2026-05-22
**Status**: Targeted recovery complete, no local commit created

### Summary

- Recovered the repo state after a machine reboot instead of assuming any prior local services were still running.
- Confirmed the working tree was already dirty before this pass with unrelated cockpit/browser-stability edits plus the feed-monitor slice still modified in `apps/web/tests/feed-monitor.spec.ts`, `docs/implementation-matrix.md`, `docs/regression-qa-matrix.md`, and `docs/build-ledger.md`.
- Restarted the local stack on `127.0.0.1:8000` and `127.0.0.1:3000`, then corrected the API startup mode to `APP_ENV=test` after observing that the default development startup enabled APScheduler background jobs and triggered unwanted `yfinance` traffic.
- Re-ran the feed-monitor browser slice successfully on the restored stack: mocked feed-monitor coverage passed (`3/3`) and targeted route/smoke/responsive coverage passed (`5/5`).
- Re-ran the non-browser gates successfully: backend Ruff passed, backend pytest passed (`2314/2314`), frontend lint passed, frontend build passed, learning tests passed (`99/99`), and the theme-token grep gate found no new raw colour violations.
- Ran the broader Playwright suite on the restored stack and stopped once the failure pattern was conclusive. The full suite remained broadly red well outside the feed-monitor slice, so this recovery pass does not justify a commit.

### Commands Run

- `cd apps/api && APP_ENV=test POSTGRES_HOST=localhost POSTGRES_PORT=5432 POSTGRES_USER=postgres POSTGRES_PASSWORD=postgres POSTGRES_DB=market_hunter BROKER_MODE=paper LIVE_EXECUTION_ENABLED=false .venv/bin/python -m uvicorn app.main:app --host 127.0.0.1 --port 8000`
- `cd apps/web && NEXT_PUBLIC_API_BASE_URL=http://127.0.0.1:8000 npm run dev -- --hostname 127.0.0.1 --port 3000`
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/feed-monitor.spec.ts --reporter=line`
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/routes.spec.ts tests/responsive.spec.ts tests/smoke.spec.ts --grep 'feed monitor|feed-monitor|/monitor/feeds' --reporter=line`
...
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/broker-health-and-control.spec.ts --grep 'trading control panel shows live configured but locked wording' --reporter=line`
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/regression.spec.ts --grep 'QA-003 analytics route loads and key panels render' --reporter=line`
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/full-flow.spec.ts --grep 'signals page loads live feed and recent news' --reporter=line`

### Validation Results

- Feed-monitor mocked browser coverage: **3/3 passed**
- Feed-monitor route/smoke/responsive coverage: **5/5 passed**
- Backend Ruff: **pass**
- Backend pytest: **2314/2314 passed**
- Frontend lint: **pass**
- Frontend build: **pass**
- Learning suite: **99/99 passed**
- Theme-token gate: **pass**

### Full Playwright Classification

- **A. Caused by feed-monitor changes**: none confirmed. The feed-monitor slice stayed green in isolated reruns after the reboot.
- **B. Environment or stack after reboot**: confirmed in part. The first API restart used development mode and enabled scheduler traffic; restarting under `APP_ENV=test` removed that source of external churn. Feed-monitor remained green after that correction.
- **C. Stale visual snapshots**: present. The interrupted full run reproduced snapshot-size and screenshot diffs on analytics, execution, performance, and assets mobile visual baselines. No snapshot updates were made in this pass.
- **D. Unrelated broker and adjacent feature surfaces**: present. Broker health/control, broker provenance/audit, broker readiness history, broker submit/dry-run, cockpit mode, and strategy-lab assertions failed well outside the feed-monitor slice.
- **E. Broader repo regressions or unstable current workspace state**: present. Analytics loading, nav/main-layout visibility, execution/smoke flows, approvals/risk flows, signals recent-news rendering, and multiple responsive overflow/tap-target checks failed during the broader suite.

### Known Limitations

- The full Playwright run was stopped after the failure clusters were already conclusive; the interrupted capture had already reached `[247/292]` with the failure index at `140`, so the suite is unambiguously red even without a final terminal summary line.
- The current working tree still contains unrelated local changes outside the feed-monitor slice, especially the cockpit/browser-stability files recovered at the start of this pass. They were not altered or staged here.
- Because the broader Playwright suite is red across unrelated surfaces and the working tree is mixed, this recovery pass is not commit-safe.

### Commit Decision

- No local commit created in MH-FEED-MONITOR-002-R1.
- The targeted feed-monitor recovery threshold is met, but the broader suite is red outside this slice and the working tree contains unrelated local feature edits, so staging or committing would mix scopes unsafely.

### Next Phase

→ Triage the unrelated broader Playwright failures by cluster before any push or commit attempt: broker surfaces, analytics/loading and navigation stability, strategy-lab state coverage, responsive overflow/tap targets, and stale visual baselines.

---

## MH-PLAYWRIGHT-RECOVERY-001 — Execution Route Sync Browser Gate Recovery

**Date**: 2026-05-22
**Status**: Validation complete, local commit pending staging review

### Summary

- Cleared the remaining unrelated browser-gate blocker after the MH-FEED-MONITOR-002 recovery had already re-confirmed the feed-monitor slice on the restored local stack.
- Root cause: the execution-page URL-sync effect continued mutating query params after navigation away from `/execution` had started, which could race the sidebar route change and rewrite the browser back to `/execution?executionId=...&asset=...`.
- Updated `apps/web/lib/hooks/useExecutionPageController.ts` so query-param synchronization only runs while the current pathname is `/execution`, and so the orphaned `asset` query param is removed when no execution is selected.
- No broker, live-trading, or execution-safety policy behavior changed. The fix stays inside client-side route ownership for the execution page.
...
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/full-flow.spec.ts --grep "navigates from Dashboard → Execution → Analytics via sidebar" --reporter=line`
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/feed-monitor.spec.ts --reporter=line`
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test tests/routes.spec.ts tests/responsive.spec.ts tests/smoke.spec.ts --grep 'feed monitor|feed-monitor|/monitor/feeds' --reporter=line`
- `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3000 ./node_modules/.bin/playwright test --reporter=line`
- `cd apps/api && .venv/bin/ruff check app tests`
...
- Targeted execution sidebar navigation coverage: **1/1 passed**
- Feed-monitor mocked browser coverage: **3/3 passed**
- Feed-monitor route/smoke/responsive coverage: **5/5 passed**
...
- Frontend build: **pass**
- Learning suite: **99/99 passed**
- Theme-token grep gate: **pass**

### Relationship To MH-FEED-MONITOR-002

- MH-FEED-MONITOR-002 had already restored and revalidated the feed-monitor browser slice after reboot.
- This follow-up recovery fixed the separate execution-route sync regression that was still holding the broader browser gate open, bringing the full Playwright suite back to green at **292 passed**.
...
## Manual IBKR Paper Submit Readiness Review

- **Date:** 2026-05-23
- **Status:** ✅ Complete
- **Scope:** Add the narrowest safe operator-facing readiness review layer on top of the existing recommendation route-check and guarded broker dry-run preview. This phase is review-only, non-submitting, adds no submit button, keeps workers non-submitting, and leaves live locked.

### UI Surface Changed

- `/cockpit/in-flight-adjustments` now renders a read-only `Manual IBKR paper submit readiness` section inside `RecommendationRouteCheckPanel`.
- The readiness section appears only as operator review visibility after route-check is loaded, then updates as guarded dry-run evidence becomes available.
- It answers whether the route-check passed, whether guarded dry-run passed, whether broker mode is coherently paper, whether the resolved route is still `/broker/orders`, whether preflight is non-blocking, whether source labels are correct, whether live is still locked, whether workers remain non-submitting, and whether the recommendation is ready for a future manual IBKR paper submit handoff.

### API Helper / Component Added

- No new backend API helper was required.
- `apps/web/components/RecommendationRouteCheckPanel.tsx` now derives readiness frontend-only from the existing route-check and guarded dry-run preview contracts via a dedicated helper.
- `apps/web/components/RecommendationRouteCheckPanel.module.css` adds minor readiness display styling.
- `apps/web/tests/in-flight-adjustments.spec.ts` now covers readiness states for `dry_run_required`, `blocked`, `missing_context`, and `ready_for_future_manual_paper_submit`.

### Backend Changed Or Not

- No backend production code changed in this phase.
- No new readiness endpoint was added because the existing route-check and guarded dry-run preview already contained the required evidence.
...
- Readiness review is non-submitting.
...
- Readiness is derived in the frontend from existing read-only evidence; it does not create a new server-side readiness contract.
- Even when readiness is green, no order is submitted and no submit control is rendered.
...
## Manual Paper Submit Audit/Handoff Package

- **Date:** 2026-05-23
- **Status:** ✅ Complete
- **Scope:** Add the narrowest safe operator-facing audit/handoff package on top of the existing recommendation route-check, guarded broker dry-run preview, readiness review, and handoff review. This phase remains review-only, non-submitting, adds no submit button, adds no `/broker/orders` call from the in-flight surface, keeps workers non-submitting, and leaves live locked.

### UI Surface Changed

- `/cockpit/in-flight-adjustments` now renders a read-only `Manual paper submit audit package` section inside `RecommendationRouteCheckPanel` after the handoff review.
- The audit package consolidates recommendation identity, symbol, side, quantity/notional context, order type, route-check result, guarded dry-run result, readiness review result, handoff review result, source labels, decision references, safety state, payload preview fields, blocked reasons, missing context, warnings, next required action, and explicit no-order-submitted confirmation.
- The panel stays review-only and explicitly states that no submit button is available here, no `/broker/orders` call is made from this surface, live remains locked, and workers cannot submit.

### Package Helper / Component Added

- No new backend API helper was required.
- `apps/web/components/RecommendationRouteCheckPanel.tsx` now derives a frontend-only `deriveManualPaperSubmitAuditPackage(...)` helper from the existing route-check, guarded dry-run preview, readiness evidence, and handoff evidence.
- The same component now tightens the handoff review to surface `handoff_required` when advisory dry-run evidence still needs operator review before the package can be considered ready.
- `apps/web/tests/in-flight-adjustments.spec.ts` now covers audit-package states for `dry_run_required`, `readiness_required`, `handoff_required`, `blocked`, `missing_context`, and `package_ready_for_future_manual_review`.

### Backend Changed Or Not

- No backend production code changed in this phase.
- No read-only audit package endpoint was added because the existing route-check and guarded broker dry-run preview already exposed the required review evidence.
- No call to `BrokerService.submit_order` was added.
- No call to `/broker/orders` was added from the frontend in this phase.
...
- Audit package is non-submitting.
- No submit button was added.
- No `/broker/orders` call was added from this UI block.
- No submit controls were added.
- No order was submitted.
...
- Frontend lint: `cd apps/web && npm run lint` -> passed
- Frontend build: `cd apps/web && npm run build` -> passed
- Targeted browser regression slice: `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 ./node_modules/.bin/playwright test tests/routes.spec.ts tests/responsive.spec.ts tests/smoke.spec.ts --grep 'recommendation|route-check|dry-run|readiness|handoff|audit package|IBKR paper|broker dry-run|manual paper|in-flight' --reporter=line` -> `4 passed`
- TSX colour gates:
  - `grep -RniE '#[0-9A-Fa-f]{3,6}\b' apps/web/app apps/web/components --include='*.tsx'`
  - `grep -RniE 'rgba?\s*\(' apps/web/app apps/web/components --include='*.tsx'`
  - no matches
...
- Audit package is derived in the frontend from existing read-only evidence; it does not create a new server-side audit package contract.
- Even when the package is ready for future manual review, no order is submitted and no submit control is rendered.
...
## Final Guarded Operator Submit Interaction Spec

- **Date:** 2026-05-23
- **Status:** ✅ Complete
- **Scope:** Add the narrowest safe read-only final interaction spec layer on top of the existing recommendation route-check, guarded broker dry-run preview, readiness review, handoff review, audit package, approval package, preflight contract, design review, submit-decision review, and operator action review. This block remains spec/review only, keeps `action_available_now=false`, writes no decision now, adds no submit button, adds no `/broker/orders` UI call, adds no new `BrokerService.submit_order` path, keeps workers non-submitting, and leaves live locked.

### Final Interaction Spec Contract

- **Future interaction name:** `manual_ibkr_paper_submit`
- **Future route later:** guarded `/broker/orders` only when the full paper-mode safety posture remains coherent
- **Action enabled now:** `false`
- **Interaction spec review only:** `true`
- **Decision write performed now:** `false`
- **Submitted order:** `false`
- **Submit-time reruns required later:** `true`
- **Operator confirmations required later:** `true`
...
- `/cockpit/in-flight-adjustments` now renders a read-only `Final guarded operator submit interaction spec` section inside `RecommendationRouteCheckPanel` after the operator action review.
- The section shows the future manual IBKR paper interaction name, the later guarded `/broker/orders` route only when safe, current review gates passed, review gates still blocking, final operator confirmations required later, final payload preview fields, submit-time checks that rerun later, future decision records, later interaction sequence steps, read-only lock states, and explicit `no order submitted` / `action available now: false` safety state.
- This is review-only. No decision is written now. No submit button was added. No `/broker/orders` UI call was added.
...
### Final Interaction Spec Statuses Implemented
...
- Live remains locked.
- Actual future submit still uses the existing guarded `/broker/orders` path.
- This is operator interaction visibility, not automation.
...
- Frontend build: `cd apps/web && npm run build`
- Focused Playwright: `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 ./node_modules/.bin/playwright test tests/in-flight-adjustments.spec.ts --reporter=line`
- Targeted browser regression slice: `cd apps/web && PLAYWRIGHT_BASE_URL=http://127.0.0.1:3100 ./node_modules/.bin/playwright test tests/routes.spec.ts tests/responsive.spec.ts tests/smoke.spec.ts --grep 'recommendation|route-check|dry-run|readiness|handoff|audit package|approval package|preflight contract|design review|submit-decision|operator action|final guarded submit|IBKR paper|broker dry-run|manual paper|in-flight' --reporter=line`
- TSX colour gates: `grep -RniE '#[0-9A-Fa-f]{3,6}\b' apps/web/app apps/web/components --include='*.tsx'` and `grep -RniE 'rgba?\s*\(' apps/web/app apps/web/components --include='*.tsx'`
- Full backend regression: `cd apps/api && .venv/bin/python -m pytest tests/ -q`
- Learning validation: `cd /Users/ants/Documents/market-hunter-mvp && scripts/test/test-learning.sh`
...
- The new section is derived from existing review-chain evidence and static future-interaction rules; it does not fetch or write any interaction state.
- `operator_action_review_required` is a defensive interaction-spec status for future drift or partial wiring; the current happy-path evidence reaches `interaction_spec_ready_for_future_phase` only as a visibility state and still never enables action.
- No execution seam is added in this block.
- Any future enablement phase must still keep `/broker/orders` as the only guarded submit seam, preserve append-only decision persistence, keep workers non-submitting, and keep live locked unless a separate safety phase explicitly changes that contract.
...
## Timeline Page Body SHA + Audit Hub Count Contract Pins

- **Scope:** Additive drift-lock pins layered on top of the previous timeline SHA / cockpit-link block. SHA-256-pins the full body of the timeline page React component (`apps/web/app/cockpit/audit/broker-submit-decisions/page.tsx`) and pins the contract that the cockpit audit hub's broker-submit-decisions tile derives its row-count display from the audit envelope's `count` field (never `items.length`). No production code changed; no submit behaviour added; no new routes; no roadmap shift.
- **Backend page-body SHA pin added (`apps/api/tests/test_broker_submit_decision_timeline_frontend_drift_lock.py`, +1 test):**
  - `test_submit_decisions_timeline_page_body_hash_is_pinned` SHA-256-pins the full text of `apps/web/app/cockpit/audit/broker-submit-decisions/page.tsx` at `18af398acafee3b81c41a97a95b3bbcfdec3c2382fdd0b27a9dd6daf4174ba87` (len 16723). Failure message instructs the operator to re-verify read-only/submit-free posture (no `/broker/orders`, no `/execution/paper`, no `submitBrokerOrder`/`cancelBrokerOrder`/`submitOrder`) before updating the hash.
- **Audit-hub count contract pin added (same file, +1 test):**
  - `test_audit_hub_broker_submit_decisions_count_uses_envelope_count` extracts the broker-submit-decisions tile block from `apps/web/app/cockpit/audit/page.tsx` by anchoring at the `/cockpit/audit/broker-submit-decisions` href, then balanced-brace scanning the `loadCount: async () => { ... }` arrow body. Asserts the block contains `getRecentBrokerSubmitDecisions` and `resp.count`, and that it does NOT contain forbidden derivations `resp.items.length`, `resp.items?.length`, `resp?.items?.length`, `response.items.length`, `resp.total`, `resp.size`, `resp.length`. A silent switch to any of these would zero the tile.
- **Cockpit audit landing link pin status:** Already covered by the previous block's `test_cockpit_audit_index_links_to_timeline_page`, `test_cockpit_audit_index_tile_advertises_broker_submit_decisions`, and `test_cockpit_audit_index_uses_audit_client_helper`. The new count-contract pin extends but does not replace those.
- **Adjacent audit feed coverage — deferred:** Routes `apps/api/app/api/routes/risk_decisions.py`, `news_in_decision_log.py`, and `llm_logs.py` use bare `@router.get("/recent")` decorators with no `response_model=` annotation; the audit-hub helper for worker runs is served by `monitor_worker_run_log.py` (also no `response_model=`). Extending `test_response_model_catalog_drift_lock.py`'s scan to cover them would either produce silent absences (no annotation to scan) or require adding `response_model=` to production routes, which is out of scope for a drift-lock-only block. Deferred as future candidates: (a) introduce typed `response_model` bindings on the four sibling audit feeds in a normal feature block, then add them to the catalog; (b) add a no-secret-fields scan over their persisted-row schemas once response models exist.
- **Explicit non-changes:**
  - No production code touched. Zero edits under `apps/api/app/`, `apps/web/app/`, `apps/web/components/`, `apps/web/lib/`.
  - No mutation route added; no submit behaviour added.
  - Auto trading remains OFF; live trading remains locked; workers remain non-submitting.
...
- **Validation commands and results:**
  - `cd apps/api && .venv/bin/ruff check app tests` -> `All checks passed!`.
  - `cd apps/api && .venv/bin/python -m pytest tests/test_broker_submit_decision_timeline_frontend_drift_lock.py tests/test_broker_submit_decision_timeline_route_surface_drift_lock.py tests/test_response_model_catalog_drift_lock.py -q` -> `30 passed in 0.61s` (was 28; +2 new).
  - `cd apps/api && .venv/bin/python -m pytest tests/ -q` -> `2437 passed in 302.16s` (was 2435; +2 new).
  - `./scripts/test/test-learning.sh` -> `99 passed`.
  - `git diff --check` -> clean.
- **Locked invariants confirmed:**
  - Any change to the timeline page body now requires a deliberate hash update with explicit read-only/submit-free re-verification.
  - The cockpit audit hub cannot silently zero the broker-submit-decisions tile by swapping its count derivation off the audit envelope's `count` field.
...
## Paper Submit Result UX and Operator Outcome View

- **Scope:** Frontend UX-only improvement to the existing guarded IBKR paper submit confirmation page. Adds a dedicated post-attempt operator outcome view so the operator can clearly read whether the manual paper submit was allowed, blocked, or failed without scrolling through pre-attempt review context. No backend, no new submit route, no new submit behaviour. Live remains locked, workers remain non-submitting, auto-trading remains OFF, and `/broker/orders` remains the only serious-paper submit seam.
- **Files changed (frontend only):**
  - `apps/web/app/cockpit/manual-paper-submit-confirmation/page.tsx` — adds a `SubmitAttemptRecord` captured at submit time, a typed outcome status (`allowed` / `blocked` / `failed`), and a new `OperatorOutcomeView` section (`data-testid="manual-paper-submit-outcome-view"`).
  - `apps/web/styles/pages/manual-paper-submit-confirmation.module.css` — adds a small `.outcomeHeader` layout helper for the outcome section's title row.
  - `apps/web/tests/manual-paper-submit-confirmation.spec.ts` — extends the existing happy-path test with full outcome-view assertions and adds two new tests: a blocked-submit outcome-view assertion (driven by the existing structured 403 fixture) and a failed-submit outcome-view assertion (driven by a generic 500 broker response). The existing `manual-paper-submit-success-state` and `manual-paper-submit-error-state` selectors continue to pass unchanged.
- **Result UX fields shown in the outcome view:**
  - Outcome status badge (`allowed` / `blocked` / `failed`) and `data-outcome-status` attribute.
  - Persistent safety badges: Paper only, Live remains locked, Workers cannot submit, No live order was placed.
  - Attempt details: symbol, side, quantity, order type, time in force, limit price (or "not required"), estimated notional, recommendation id, correlation id (= `submit_decision_correlation_id` = `client_order_id`), and the operator-visible attempt timestamp.
  - Guard result: outcome status, broker mode (from broker response when available, falling back to dry-run/route-check evidence), broker account mode, execution source, preflight decision status, dry-run `allowed_to_submit` and `would_block`, broker response status, broker order id, filled qty / price, live_execution_enabled, paper_trading_enabled, and (when the failure was structured) `submit_gate` and `response_decision_status`. Route-check and dry-run reference strings are echoed for traceability.
  - Blocked reasons list (or an empty-state message when none were returned).
  - Safe warning paragraph echoing the broker/API error message for non-allowed outcomes; raw broker payloads, secrets, tokens, and bearer-style fields are not rendered.
- **Timeline link:** A read-only link `/cockpit/audit/broker-submit-decisions?correlation_id={correlation}&recommendation_id={recommendationId}` is rendered as "View full submit decision timeline". The link points at the existing append-only decision-timeline page (still SHA-pinned and submit-free) — it does not add a new route, mutation, or rerun control. Two adjacent navigation links also point back to the in-flight review for the originating recommendation and to the cockpit hub.
- **Next-step guidance per outcome:**
  - Allowed: "Review timeline and monitor paper account" — review the timeline, monitor the IBKR paper account, return to the cockpit hub.
  - Blocked: "Resolve blockers and rerun review" — open the timeline, fix missing context, rerun the dry-run before any further paper submit attempt, return to in-flight review.
  - Failed: "Check broker/API status and timeline before retry" — check broker/API status, open the timeline, rerun the dry-run, return to in-flight review.
  - All three explicitly state "No automatic resubmission will occur. Live trading remains locked. Workers remain non-submitting." There is no one-click retry-submit control.
- **Safety invariants:**
  - The submit path still calls `/broker/orders` exactly once per operator click. The outcome view itself never issues a network call.
  - No new submit route, no live unlock, no auto-submit, no worker submit, no hidden resubmission. `assert_auto_trading_allowed()` remains unconditional. `/broker/orders` remains the only serious-paper submit seam; `/execution/paper` remains simulator-only.
  - Live wording is explicitly excluded — the Playwright tests still assert that no control matching `trade now|auto submit|approve live|one-click execute|live submit|buy now|sell now|execute now` exists on the page.
  - No raw broker payload is rendered: the outcome view only echoes structured fields already exposed on the read-only review chain plus the explicitly typed `BrokerOrderResult` and `paper_preflight_blocked` detail envelope. Credential-shaped tokens (`api[_-]?key`, `secret`, `password`, `bearer`) are explicitly asserted absent on the failed-outcome test.
  - The broker submit decision timeline remains read-only and submit-free (its SHA-pin/no-submit-imports tests are untouched). The link uses GET-only query parameters that the existing timeline route already accepts.
- **No new submit behaviour:** Submit logic is unchanged. The only new state is local React state capturing the attempted payload's display fields and an attempt timestamp; the network call surface is the same `submitBrokerOrder` already in place.
- **Validation results (recorded in commit):** Frontend lint, frontend build, targeted Playwright reruns of `manual-paper-submit-confirmation.spec.ts` and `broker-submit-decisions-timeline.spec.ts`, and a broader cockpit grep over `routes.spec.ts`, `responsive.spec.ts`, `smoke.spec.ts` were run. Backend was untouched, so backend `ruff` + targeted broker + full pytest were run as a regression sanity check only. `scripts/test/test-learning.sh` and `git diff --check` were run as hygiene gates.
- **Next recommended phase:** Operator outcome timeline deep-link verification on the timeline page itself (so following the outcome's "View full submit decision timeline" link lands directly on the recorded preflight + attempt rows with the correlation-id filter pre-applied and visible), or — if instead resuming drift-lock cycles — a SHA-pin on the new `OperatorOutcomeView` body to lock the post-attempt safety wording (Paper only / Live remains locked / Workers cannot submit / No live order was placed / No automatic resubmission will occur).
